...
async function startConsumer(groupId, topics, onMessage) {
  // Retry the entire connect+subscribe cycle so transient "topic-partition"
  // metadata errors (common on fresh Kafka startup) are automatically healed.
  for (let attempt = 1; attempt <= 15; attempt++) {
    const consumer = kafka.consumer({ groupId });
    try {
      await consumer.connect();

      // Small pause so Kafka partition metadata is fully propagated
      ...
      console.log(
        `[kafka] compliance-service consumer ready — topics: ${topics.join(", ")}`,
      );
