...
export default function SellerDashboard() {
  ...
  /* ── Fetch seller's own listings via /listings/my ── */
  ...
  const closeEditModal = () => {
    // Only revoke blob URLs (not the existing server URL)
  ...
  const handleEditSave = async (e) => {
    ...
    try {
      ...
      await loadListings();
      setTimeout(() => { closeEditModal(); setEditSuccess(""); }, 1000);
    } catch (err) {
            ...
            <form onSubmit={handleEditSave} className="space-y-4">
              ...
              <div>
                ...
                <label className={`inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-xl border cursor-pointer transition-all ${
                  ...
                  {editImageFile ? editImageFile.name : editListing?.image_url ? "Replace image…" : "Choose image…"}
