...
export default function ShopPage() {
  ...
  const handlePurchase = async (listingId) => {
    ...
    try {
      ...
      setToast({
        type: "success",
        message: `Purchase successful! Order status: ${data.order?.status}`,
      });

      // Update local listing status so the button disappears
      setListings((prev) =>
        prev.map((l) => (l.id === listingId ? { ...l, status: "pending" } : l)),
      );
    } catch (err) {
  ...
  const inputBase =
    "px-4 py-2.5 rounded-xl bg-white/[0.04] border border-white/10 text-slate-100 placeholder-slate-600 text-sm focus:outline-none focus:border-emerald-500 focus:shadow-[0_0_0_3px_rgba(16,185,129,0.12)] transition-all";
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1a] pt-24 pb-20 px-6 md:px-10">
      ...
      <div className="max-w-7xl mx-auto flex flex-col sm:flex-row gap-3 mb-8">
        ...
        >
          {TYPE_OPTIONS.map((t) => (
            <option key={t} value={t} className="bg-[#1a2540] capitalize">
              {t}
            </option>
          ))}
        </select>
        ...
        >
          <option value="newest" className="bg-[#1a2540]">
            Newest First
          </option>
          <option value="oldest" className="bg-[#1a2540]">
            Oldest First
          </option>
          <option value="price_asc" className="bg-[#1a2540]">
            Price: Low → High
          </option>
          <option value="price_desc" className="bg-[#1a2540]">
            Price: High → Low
          </option>
        </select>
      </div>
      ...
      {toast && (
        <div className="fixed bottom-8 right-8 z-50 animate-bounce-short">
          <div
            className={`px-5 py-4 rounded-2xl border shadow-[0_12px_48px_rgba(0,0,0,0.5)] flex items-center gap-3 ${
              toast.type === "success"
