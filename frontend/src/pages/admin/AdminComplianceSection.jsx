...
const FIELDS = [
  ...
  },
  {
    ...
    type: "text",
    required: true,
    hint: "The seller's user UUID from the Identity Service.",
  },
  {
    name: "buyerEmail",
    label: "Buyer Email",
...
export default function AdminComplianceSection() {
  ...
  const [history, setHistory] = useState([]); // last 5 checks in session
      ...
      <div className="flex items-start justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            Manually verify whether a species is permitted for sale on this
            platform.
          </p>
        </div>
        {result && (
      ...
      <div className="grid lg:grid-cols-[1fr_420px] gap-6 mb-6">
        ...
        >
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3 rounded-xl bg-emerald-700 hover:bg-emerald-600 active:bg-emerald-700 disabled:opacity-50 text-white font-bold text-sm transition-all flex items-center justify-center gap-2 shadow-[0_4px_16px_rgba(16,185,129,0.15)]"
          >
            {loading ? (
              <>
            ...
            >
              ...
              <div
                className={`flex items-center gap-3 p-4 rounded-xl mb-5 ${
                  isAllowed ? "bg-emerald-500/10" : "bg-red-500/10"
                }`}
              >
